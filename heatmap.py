...
class yolo_heatmap:
    ...
    def process(self, img_input, save_path):
        # img process
        try:
            if isinstance(img_input, str):
                ...
                if img is None:
                    # Try imdecode for non-ASCII paths
                    img = cv2.imdecode(np.fromfile(img_input, np.uint8), cv2.IMREAD_COLOR)
            ...
            if img is None:
                raise ValueError(f"Failed to load or convert image")
                
        except Exception as e:
            print(f"Warning... Image processing failure: {e}")
            return None
    ...
    def __call__(self, img_input, save_path=None):
        if isinstance(img_input, str) and os.path.isdir(img_input):
            # Process a directory of images
            results = []
            for img_file in os.listdir(img_input):
                img_file_path = os.path.join(img_input, img_file)
                if save_path:
                    os.makedirs(save_path, exist_ok=True)
                    save_file_path = os.path.join(save_path, img_file)
                else:
                    save_file_path = None
                result = self.process(img_file_path, save_file_path)
                if result is not None:
                    results.append(result)
            return results
        else:
            # Process a single image
            return self.process(img_input, save_path)
